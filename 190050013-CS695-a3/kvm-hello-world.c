...
void print_timespec(const struct timespec *ts) {
    fprintf(stdout, "%ld.%09ld", ts->tv_sec, ts->tv_nsec);
}
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;) {
		...
		num_exists++;
		// fprintf(stdout, "%d %d %d\n", vcpu->kvm_run->exit_reason, KVM_EXIT_IO, KVM_EXIT_HLT);
		...
		case KVM_EXIT_IO:
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    ...
			    && vcpu->kvm_run->io.port == 0xEE) {
				char *p = (char *)vcpu->kvm_run;
				*((uint32_t *)(p + vcpu->kvm_run->io.data_offset)) = num_exists;
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				struct kvm_clock_data clock_data = { 0 };
				int ret;

				ret = ioctl(vm->fd, KVM_GET_CLOCK, &clock_data);
				...
				uint32_t timestamp_lsb = (uint32_t) clock_data.clock;
				char *p = (char *)vcpu->kvm_run;
				*((uint32_t *)(p + vcpu->kvm_run->io.data_offset)) = timestamp_lsb;
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				if (ioctl(vcpu->fd, KVM_TRANSLATE, &translation) < 0) {
				...
				uint32_t hva = (uint32_t)(uint64_t)(vm->mem + translation.physical_address);
				mapped_addr = hva;
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
			    && vcpu->kvm_run->io.port == 0xE8) {
				char *p = (char *)vcpu->kvm_run;
				*((uint32_t *)(p + vcpu->kvm_run->io.data_offset)) = mapped_addr;
				continue;
			}
			/* fall through */
		default:
